...
def home():
    ...
    if(user!=""):
        ...
        if(stdname2=="전체보기"):        
            ...
            twofile_list = datalist.user_load_list(stdname2, user)  #이미지 리스트 불러오기
            twolength=len(twofile_list)
            two_list=""
            twolength2=len(two_list)
            ...
            two_list = twoimagelist.user_load_list(stdname2, user)  #이미지 리스트 불러오기
...
def create_info():
    ...
    #필터에 따른 생성모델로 이동
    if(filter=="CompVis"):
        std_CompVis.compvis(ptranslated_text, cfg, steps)
        situation=tt.test(ptranslated_text, ntranslated_text, filter, "static/testimg/"+ptranslated_text+".jpeg")
    else:
        std_tutorial.stdv1_5(ptranslated_text, ntranslated_text, filter, cfg, steps)     
        situation=tt.test(ptranslated_text, ntranslated_text, filter, "static/testimg/"+ptranslated_text+".jpeg")
    ...
    if(situation==0):
        ...
        datalist.save(datalist.Now_idx(),user,stdname,ptranslated_text,ntranslated_text,filter,"X","X",cfg, steps, result_file, share) 
        ...
        return redirect(f"/can't_create/1/?pprompt={pprompt}&nprompt={nprompt}&filter={filter}&cfg={cfg}&steps={steps}&noise=_")
    elif(situation==2):
        return redirect(f"/can't_create/2/?pprompt={ptranslated_text}&nprompt={ntranslated_text}&filter={filter}&cfg={cfg}&steps={steps}&noise=_")
...
def detail(index):
    ...
    return render_template("detail_info.html", pprompt=pprompt, nprompt=nprompt, filter=filter, file=file, cfg=cfg, steps=steps, noise=noise, photo=photo)
...
def detail2(index):
    ...
    photo= "/"+d_info[6]
    return render_template("detail_info2.html", pprompt=pprompt, nprompt=nprompt, filter=filter, file1=file1, file2=file2, photo=photo)
...
def change_image():
    ...
    cfg=float(request.form.get('cfg'))
    ...
    std_img_img.img_to_img(ptranslated_text,ntranslated_text, filter, used_image_path, cfg, steps, noise)
    situation=tt.test(ptranslated_text,ntranslated_text, filter, "static/testimg/"+ptranslated_text+".jpeg")
    if(situation==0):
        image_path = f"static\img\{datalist.Now_idx()}.jpeg"  # 이미지 파일 경로
        image = Image.open(image_path)  # 이미지를 로드합니다.
        ...
        datalist.save(datalist.Now_idx(),user,stdname, ptranslated_text, ntranslated_text, filter, used_image_path, noise, cfg, steps, image_url, share) 
        response_data={"prompt":ptranslated_text}
        image_file=f"img/{datalist.Now_idx()-1}.jpeg"
        return jsonify(response_data=response_data, image_file=image_file) 
    elif(situation==1):
        return redirect(f"/can't_create/1/?pprompt={image_prompt_text}&nprompt={image_nprompt_text}&filter={filter}&cfg={cfg}&steps={steps}&noise={noise}")
    elif(situation==2):
        return redirect(f"/can't_create/2/?pprompt={ptranslated_text}&nprompt={ntranslated_text}&filter={filter}&cfg={cfg}&steps={steps}&noise={noise}")
...
def inpainting_image():
    ...
    filter=f
    inpainting.in_painting(ptranslated_text, ntranslated_text, filter, image_path, mask_image_path,user,stdname, share)
...
def openpose_image():    
    ...
    open_pose.openpose(ptranslated_text, ntranslated_text, filter, image_path,user,stdname, share)
    print(filter)
    response_data={"prompt":ptranslated_text}
...
def fail(situation):
    ...
    if red==2:
        situation=situation
        application.config["RED"]=0
        return render_template("fail.html", index=situation, pprompt=pprompt, nprompt=nprompt, filter=filter, cfg=cfg, steps=steps, noise=noise)
    else:
